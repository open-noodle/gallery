...
describe('GET /admin/face-repair/faces/:assetFaceId/preview', () => {
  ...
  beforeAll(async () => {
    ...
    memberFaceId = await utils.createFace({ assetId: memberAsset.id, personGroupId: person.id });

    const trashedAsset = await utils.createAsset(member.accessToken);
    trashedFaceId = await utils.createFace({ assetId: trashedAsset.id, personGroupId: person.id });
