...
<div
  class={[
    'flex h-[52px] items-center gap-3 rounded-lg px-3 py-2 transition-colors duration-80 ease-out group-data-selected:bg-primary/10',
    isPending && 'opacity-50',
  ]}
>
  {#if thumbUrl}
    <img src={thumbUrl} alt="" class="size-10 rounded-md object-cover" loading="lazy" />
  {:else}
    <div class="flex size-10 items-center justify-center rounded-md bg-subtle/40" aria-hidden="true">
  ...
  {#if isPending}
    <span
      ...
      class="size-4 shrink-0 rounded-full border-2 border-gray-300 border-t-primary dark:border-gray-600 dark:border-t-primary"
