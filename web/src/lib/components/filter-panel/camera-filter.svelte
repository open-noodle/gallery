<script lang="ts">
  interface Props {
    makes: string[];
    selectedMake?: string;
    selectedModel?: string;
    onModelFetch: (make: string) => Promise<string[]>;
    onSelectionChange: (make?: string, model?: string) => void;
  }

  let { makes, selectedMake, selectedModel, onModelFetch, onSelectionChange }: Props = $props();

  let expandedMake = $state<string | undefined>(undefined);
  let models = $state<string[]>([]);
  let loadingModels = $state(false);

  $effect(() => {
    if (expandedMake) {
      loadingModels = true;
      onModelFetch(expandedMake).then((result) => {
        models = result;
        loadingModels = false;
      });
    } else {
      models = [];
    }
  });

  function handleMakeClick(make: string) {
    if (selectedMake === make && !selectedModel) {
      // Deselect make
      expandedMake = undefined;
      onSelectionChange(undefined, undefined);
    } else {
      // Select make
      expandedMake = make;
      onSelectionChange(make, undefined);
    }
  }

  function handleModelClick(model: string, make: string) {
    if (selectedModel === model) {
      // Deselect model, keep make
      onSelectionChange(make, undefined);
    } else {
      // Select model (auto-fills make)
      onSelectionChange(make, model);
    }
  }
</script>

<div data-testid="camera-filter">
  {#if makes.length === 0}
    <p class="text-[11px] text-[var(--fg-muted)]" data-testid="camera-empty">No cameras in this space</p>
  {:else}
    {#each makes as make (make)}
      {@const isMakeSelected = selectedMake === make}
      <!-- Make row -->
      <button
        type="button"
        class="flex w-full items-center gap-1.5 py-1 text-[11px] {isMakeSelected
          ? 'font-medium text-[var(--fg)]'
          : 'text-[var(--fg-muted)]'}"
        onclick={() => handleMakeClick(make)}
        data-testid="camera-make-{make}"
      >
        <!-- Radio indicator -->
        <div
          class="flex h-[13px] w-[13px] flex-shrink-0 items-center justify-center rounded-full border-[1.5px] {isMakeSelected && !selectedModel
            ? 'border-[var(--primary)] bg-[var(--primary)]'
            : 'border-[var(--border)]'}"
        >
          {#if isMakeSelected && !selectedModel}
            <div class="h-[5px] w-[5px] rounded-full bg-white"></div>
          {/if}
        </div>

        <!-- Label -->
        <span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-left">{make}</span>
      </button>

      <!-- Models (indented 20px when make is expanded) -->
      {#if expandedMake === make && !loadingModels}
        {#each models as model (model)}
          {@const isModelSelected = selectedModel === model && selectedMake === make}
          <button
            type="button"
            class="flex w-full items-center gap-1.5 py-1 pl-5 text-[11px] {isModelSelected
              ? 'font-medium text-[var(--fg)]'
              : 'text-[var(--fg-muted)]'}"
            onclick={() => handleModelClick(model, make)}
            data-testid="camera-model-{model}"
          >
            <!-- Radio indicator -->
            <div
              class="flex h-[13px] w-[13px] flex-shrink-0 items-center justify-center rounded-full border-[1.5px] {isModelSelected
                ? 'border-[var(--primary)] bg-[var(--primary)]'
                : 'border-[var(--border)]'}"
            >
              {#if isModelSelected}
                <div class="h-[5px] w-[5px] rounded-full bg-white"></div>
              {/if}
            </div>

            <!-- Label -->
            <span class="flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-left">{model}</span>
          </button>
        {/each}
      {/if}
    {/each}
  {/if}
</div>
