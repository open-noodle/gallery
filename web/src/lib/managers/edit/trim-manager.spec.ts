...
describe('TrimManager', () => {
  ...
  describe('onActivate', () => {
    ...
    it('should restore existing trim edits', async () => {
      await manager.onActivate({ duration: '0:00:30.000000' } as any, [
        { action: 'trim', parameters: { startTime: 5, endTime: 25 } },
      ] as any);
  ...
  describe('constrained playback', () => {
    it('should pause and seek to start when currentTime reaches endTime', async () => {
      ...
      const onTimeUpdate = (mockVideo.addEventListener as any).mock.calls.find(
        (c: any) => c[0] === 'timeupdate',
      )?.[1];
  ...
  describe('duration parsing', () => {
    ...
    it('should parse hours correctly', async () => {
      await manager.onActivate({ duration: '1:30:45.500000' } as any, []);
      expect(manager.duration).toBe(3600 + 1800 + 45 + 0.5);
    });

    it('should parse duration without fractional seconds', async () => {
      await manager.onActivate({ duration: '0:01:00' } as any, []);
