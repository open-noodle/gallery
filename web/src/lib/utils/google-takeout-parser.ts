...
function parseGps(
  ...
  if (Math.abs(latitude) > 90 || longitude < -180 || longitude > 180) {
    return { latitude: undefined, longitude: undefined };
  }
  ...
  if (geoData && geoDataExif && geoData !== geoDataExif) {
    ...
    if (
      ...
      // single-depth fallback: the recursive call passes `undefined` as the second
      // arg, so the `geoData && geoDataExif` guard cannot be true again.
...
export function detectAlbums(items: TakeoutMediaItem[], photoRoots: Set<string>): TakeoutAlbum[] {
  ...
  return [...albumMap].map(([name, itemPaths]) => ({
    name,
