...
const setup = (db?: Kysely<DB>) => {
  ...
  ctx.getMock(JobRepository).empty.mockResolvedValue();
  return { sut, ctx };
};
...
describe('PetRecognitionService.handlePetRecognition (medium)', () => {
  it('clusters two near-identical embeddings for the same owner into one pet person (5.14)', async () => {
    ...
    expect(rows[0].personGroupId).not.toBeNull();
    expect(rows[0].personGroupId).toBe(rows[1].personGroupId);

    const person = await ctx.database
      .selectFrom('person')
      .selectAll()
      .where('personGroupId', '=', rows[0].personGroupId!)
  ...
  it('produces two separate pet people for two distant embeddings (5.15)', async () => {
    ...
    expect(rows).toHaveLength(2);
    expect(rows[0].personGroupId).not.toBeNull();
    expect(rows[1].personGroupId).not.toBeNull();
    expect(rows[0].personGroupId).not.toBe(rows[1].personGroupId);

    const people = await ctx.database
      .selectFrom('person')
      .select(['personGroupId', 'species'])
      .where(
        'personGroupId',
        'in',
...
describe('PersonRepository.deleteAllPets (medium)', () => {
  ...
  it('pin: deletes an assigned pet face, its person and its pet_search row (R1.2)', async () => {
    ...
    const { assetFace: petFace } = await ctx.newAssetFace({
      assetId: asset.id,
      personGroupId: petPerson.personGroupId,
    });
    await ctx.database
      .insertInto('pet_search')
    ...
    expect(
      await ctx.database
        ...
        .where('personGroupId', '=', petPerson.personGroupId)
        .execute(),
    ).toHaveLength(0);
    expect(
  ...
  it('pin: leaves a human person, face and face_search row untouched (R1.3)', async () => {
    ...
    const { assetFace: humanFace } = await ctx.newAssetFace({
      assetId: asset.id,
      personGroupId: humanPerson.personGroupId,
    });
    await ctx.database
    ...
    const personRow = await ctx.database
      .selectFrom('person')
      .selectAll()
      .where('personGroupId', '=', humanPerson.personGroupId)
      .executeTakeFirstOrThrow();
    ...
    const faceRow = await ctx.database
      ...
      .executeTakeFirstOrThrow();
    expect(faceRow.personGroupId).toBe(humanPerson.personGroupId);
...
describe('PetRecognitionService.handleQueuePetRecognition (medium)', () => {
  ...
  it(
    ...
    async () => {
      ...
      const humanFaceRow = await ctx.database
        ...
        .where('id', '=', humanFace.id)
        .executeTakeFirstOrThrow();
      expect(humanFaceRow.personGroupId).toBe(humanPerson.personGroupId);
  ...
  it('R6.23 a pet face on a trashed asset never becomes a new pet person', async () => {
    ...
    const faceRow = await ctx.database
      ...
      .where('id', '=', face.id)
      .executeTakeFirstOrThrow();
    expect(faceRow.personGroupId).toBeNull();
  });
  ...
  it(
    ...
    async () => {
      ...
      const clusteredRows = await ctx.database
        ...
        .where('id', 'in', [faceA.id, faceB.id])
        .execute();
      const individualPersonId = clusteredRows[0].personGroupId;
      ...
      const { assetFace: birdFace } = await ctx.newAssetFace({
        assetId: birdAsset.id,
        personGroupId: birdBucket.personGroupId,
      });
      ...
      const individualPersonRows = await ctx.database
        .selectFrom('person')
        .select(['personGroupId'])
        .where('personGroupId', '=', individualPersonId!)
        .execute();
      ...
      const birdPersonRow = await ctx.database
        .selectFrom('person')
        .selectAll()
        .where('personGroupId', '=', birdBucket.personGroupId)
        .executeTakeFirstOrThrow();
      ...
      const birdFaceRow = await ctx.database
        ...
        .executeTakeFirstOrThrow();
      expect(birdFaceRow.personGroupId).toBe(birdBucket.personGroupId);
...
describe('PetRecognitionService model switch — shared-space copies (medium)', () => {
  it('R5.13b deletes only the space copy of the recognition-created individual', async () => {
    ...
    const { asset: birdAsset } = await ctx.newAsset({ ownerId: user.id });
    await ctx.newAssetFace({ assetId: birdAsset.id, personGroupId: bucket.personGroupId });
    const bucketIdentity = await ctx.get(FaceIdentityRepository).ensurePersonIdentity(bucket.personGroupId);
    ...
    const { asset: humanAsset } = await ctx.newAsset({ ownerId: user.id });
    await ctx.newAssetFace({ assetId: humanAsset.id, personGroupId: human.personGroupId });
    const humanIdentity = await ctx.get(FaceIdentityRepository).ensurePersonIdentity(human.personGroupId);
...
describe('PetRecognitionService end-to-end clustering (medium)', () => {
  it('R9.7 with minFaces: 2, the first face defers and rejoins the person the second one creates', async () => {
    ...
    const deferredRow = await ctx.database
      ...
      .where('id', '=', faceA.id)
      .executeTakeFirst();
    expect(deferredRow?.personGroupId).toBeNull();
...
describe('pet detection -> recognition pipeline (medium)', () => {
  it('R9.8 detects, embeds and clusters a dog into a pet person with species and a pet identity', async () => {
    ...
    ctx.getMock(MachineLearningRepository).detectPets.mockResolvedValue({
      imageHeight: 500,
      imageWidth: 400,
      pets: [
        {
    ...
    const faceRow = await ctx.database
      ...
      .executeTakeFirstOrThrow();
    expect(faceRow.personGroupId).not.toBeNull();

    const person = await ctx.database
      .selectFrom('person')
      .selectAll()
      .where('personGroupId', '=', faceRow.personGroupId!)
      .executeTakeFirstOrThrow();
    expect(person.type).toBe('pet');
    expect(person.species).toBe('dog');
    expect(person.ownerId).toBe(user.id);
