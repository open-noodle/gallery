...
import { AssetFileType, AssetVisibility, JobName, SharedSpaceRole, SourceType, SystemMetadataKey } from 'src/enum.js';
...
const faceRow = (ctx: ReturnType<typeof setup>['ctx'], id: string) =>
  ctx.database.selectFrom('asset_face').select(['id', 'personGroupId']).where('id', '=', id).executeTakeFirst();

const personRow = (ctx: ReturnType<typeof setup>['ctx'], id: string) =>
  ctx.database
    .selectFrom('person')
    .select(['personGroupId', 'name', 'type'])
    .where('personGroupId', '=', id)
    .executeTakeFirst();
...
describe('human face reset isolation from pet data (medium)', () => {
  it('R2.1 force recognize keeps a named pet person, its faces, embeddings and identity links', async () => {
    ...
    expect(await personRow(ctx, pet.person.personGroupId)).toMatchObject({
      personGroupId: pet.person.personGroupId,
      name: 'Rex',
      type: 'pet',
    });
    expect(await faceRow(ctx, pet.face.id)).toMatchObject({ id: pet.face.id, personGroupId: pet.person.personGroupId });
    expect(
    ...
    expect(await faceRow(ctx, human.face.id)).toMatchObject({ id: human.face.id, personGroupId: null });
  });
  ...
  it('R3.4 per-asset re-detection with zero detections keeps the pet face and drops the stale human face', async () => {
    ...
    expect(await faceRow(ctx, petFace.id)).toMatchObject({ id: petFace.id, personGroupId: petPerson.personGroupId });
    expect(
      await ctx.database.selectFrom('pet_search').selectAll().where('faceId', '=', petFace.id).execute(),
    ).toHaveLength(1);
    expect(await personRow(ctx, petPerson.personGroupId)).toMatchObject({
      personGroupId: petPerson.personGroupId,
      type: 'pet',
    });
  ...
  it('R2.5 person cleanup after a human force reset removes zero-face humans but not the pet person', async () => {
    ...
    expect(await personRow(ctx, emptyHuman.personGroupId)).toBeUndefined();
    expect(await personRow(ctx, pet.person.personGroupId)).toMatchObject({
      personGroupId: pet.person.personGroupId,
      type: 'pet',
    });
  ...
  it('R2.6 pin: person cleanup stays generic — a pet person with genuinely zero faces is collected', async () => {
    ...
    expect(await personRow(ctx, facelessPet.personGroupId)).toBeUndefined();
  });
