...
import { FaceRepairScanRepository, RepairScanParams } from 'src/repositories/face-repair-scan.repository.js';
...
const seedOverCapPerson = async (ctx: Ctx, ownerId: string, opts: { leakedCount: number; genuineCount: number }) => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const { assetFace } = await ctx.newAssetFace({
      assetId: asset.id,
      personGroupId: ownerQ.personGroupId,
...
describe('FaceRepairService.getPersonFlaggedFaces (scan-backed)', () => {
  ...
  it('filters a person-level dismiss made after the scan (E4)', async () => {
    ...
    await sut.createDeclines({
      persons: [{ personId: person.personGroupId, suspectedOwnerIds: [owner.personGroupId] }],
      declinedBy: user.id,
    });
