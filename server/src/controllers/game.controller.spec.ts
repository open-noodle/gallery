...
describe(GameController.name, () => {
  ...
  const handlerNames = Object.getOwnPropertyNames(GameController.prototype).filter((name) => name !== 'constructor');
  ...
  it.each(handlerNames)('gates %s on a Permission.Game* scope, not a shared-space one', (name) => {
    const handler = GameController.prototype[name as keyof GameController];
