...
export class PetRecognitionService extends BaseService {
  ...
  async handlePetRecognition({ id, deferred, label }: JobOf<JobName.PetRecognition>): Promise<JobStatus> {
    ...
    if (face.personGroupId) {
      ...
      await this.linkFaceIdentity(face.personGroupId, face.id);
    ...
    let personId = matches.find((match) => match.personGroupId)?.personGroupId ?? undefined;
    ...
    if (!personId) {
      ...
      personId = matchWithPerson[0]?.personGroupId ?? undefined;
    }
    ...
    await this.personRepository.reassignFaces({ faceIds: [id], newPersonGroupId: personId });
  ...
  private async handleModelSwitch(newConfig: SystemConfig, oldConfig?: SystemConfig): Promise<void> {
    await this.databaseRepository.withLock(DatabaseLock.PetRecognitionModelSwitch, async () => {
      ...
      if (state && state.modelName === newModel) {
