...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await sql`ALTER TABLE "classification_category" ADD CONSTRAINT "classification_category_name_uq" UNIQUE ("name")`.execute(db);
}

export async function down(db: Kysely<any>): Promise<void> {
  await sql`ALTER TABLE "classification_category" DROP CONSTRAINT IF EXISTS "classification_category_name_uq"`.execute(db);
  await sql`ALTER TABLE "classification_category" ADD COLUMN "userId" uuid REFERENCES "user"("id") ON UPDATE CASCADE ON DELETE CASCADE`.execute(db);
  await sql`ALTER TABLE "classification_category" ADD COLUMN "tagId" uuid REFERENCES "tag"("id") ON DELETE SET NULL`.execute(db);
  await sql`ALTER TABLE "classification_category" ADD CONSTRAINT "classification_category_userId_name_uq" UNIQUE ("userId", "name")`.execute(db);
