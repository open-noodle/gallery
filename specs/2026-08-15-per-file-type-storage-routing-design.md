...
precedent in `server/src/gallery/config.dto.ts`:
...
- `src/gallery/config.dto.ts` — `StorageRouting` enum, `GalleryStorageSchema`, `storage.routing` defaults
- `src/dtos/config.dto.ts` — composes `GalleryStorageSchema` into the admin config schema
