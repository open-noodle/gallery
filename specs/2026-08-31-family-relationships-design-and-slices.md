...
**No `updateId` column.** `@UpdateIdColumn` is the mobile sync mechanism, and `D5` states relations are deliberately not synced to Drift in this release. Adding the column would imply a sync stream that does not exist. When relations are eventually synced, adding it is a one-line migration.
...
- **New tables and indexes must be added to `scripts/revert-to-immich.sql`.** This spec adds four tables, one column and one index.
