...
void main() {
  ...
  // ---------------------------------------------------------------------------
  // Defines 13 cross-cutting permission cases that every visibility-aware
  // method (video, place, map, marker) must satisfy. The helper is invoked
  // once per method via a method-specific `insertAsset` closure that handles
  // any per-method prereqs (e.g. exif city for place, lat/lng for map/marker).
  //
  // The underscore prefix on the type/function names is stylistic — they're
  // nested inside main() so the underscore is not actually a privacy marker.
  ...
  group('Cross-method permission matrix — video()', () {
    runPermissionMatrix(
      ...
      insertAsset: (assetId, ownerId) =>
          insertVideo(assetId, ownerId, type: AssetType.video),
