...
const _tabRouteNames = ['MainTimelineRoute', 'DriftAlbumsRoute', 'SpacesRoute', 'DriftLibraryRoute'];
...
void main() {
  testWidgets('slot 1 routes to Spaces when the setting is on', (tester) async {
    ...
    expect(currentTabRoutes(tester), ['MainTimelineRoute', 'SpacesRoute', 'DriftLibraryRoute']);
  });

  testWidgets('slot 1 routes to Albums when the setting is off', (tester) async {
    await pumpShell(tester, showSpaces: false);

    expect(currentTabRoutes(tester), ['MainTimelineRoute', 'DriftAlbumsRoute', 'DriftLibraryRoute']);
  });
  ...
  testWidgets('flipping the setting while standing on slot 1 keeps the index and swaps the page', (tester) async {
    ...
    expect(activeTabRoute(tester), 'DriftAlbumsRoute');
    expect(find.byKey(const Key('tab-DriftAlbumsRoute')), findsOneWidget);
  ...
  testWidgets('the tabs the shell can host stay put when the flip lands on another slot', (tester) async {
    ...
    expect(activeTabRoute(tester), 'DriftLibraryRoute');
