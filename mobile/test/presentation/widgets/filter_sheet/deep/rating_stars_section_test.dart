...
void main() {
  group('RatingStarsSection', () {
    testWidgets('5 stars rendered regardless of suggestions', (tester) async {
      await tester.pumpConsumerWidget(const Material(child: RatingStarsSection()), overrides: _prefs());
    ...
    testWidgets('dark theme renders 5 stars', (tester) async {
      await tester.pumpConsumerWidgetDark(const Material(child: RatingStarsSection()), overrides: _prefs());
      await tester.pumpAndSettle();
      for (var i = 1; i <= 5; i++) {
        expect(find.byKey(Key('rating-star-$i')), findsOneWidget);
      }
