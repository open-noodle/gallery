...
/// `locationPresence` ('noGps' / 'noPlaceName'), `label` its already-translated row label.
/// The label is resolved at the call site rather than carried as a key, because `context.t` is a
/// static member access — there is no runtime key lookup to hand a `String` to.
class _PresenceEntry {
  final String value;
  final String label;
  const _PresenceEntry(this.value, this.label);
}
...
/// one here replaces the whole group with a fresh [SearchLocationFilter] rather than patching a
/// single field on the existing one.
///
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final entries = <_PresenceEntry>[
      if ((suggestions?.hasNoGpsAssets ?? false) || selectedPresence == 'noGps')
        _PresenceEntry('noGps', context.t.filter_location_no_gps),
