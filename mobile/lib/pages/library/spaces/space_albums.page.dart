...
class SpaceAlbumsPage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Future<void> createFolder() async {
      ...
      try {
        ...
        if (context.mounted) {
          ImmichToast.show(
            context: context,
            msg: _folderErrorMessage(context, error, context.t.space_album_folder_error_create),
    ...
    Future<void> createAlbum() async {
      ...
      try {
        ...
        if (context.mounted) {
          ImmichToast.show(
            context: context,
            msg: context.t.space_album_error_link_after_create,
            toastType: ToastType.error,
          );
        }
      }
    ...
    Future<void> renameFolder(SpaceAlbumFolder folder) async {
      final name = await _promptFolderName(
        context,
        title: context.t.space_album_folder_rename,
      ...
      try {
        ...
        if (context.mounted) {
          ImmichToast.show(
            context: context,
            msg: _folderErrorMessage(context, error, context.t.space_album_folder_error_rename),
            toastType: ToastType.error,
          );
        }
      }
    ...
    Future<void> moveFolder(SpaceAlbumFolder folder) async {
      ...
      try {
        ...
        if (context.mounted) {
          ImmichToast.show(
            context: context,
            msg: _folderErrorMessage(context, error, context.t.space_album_folder_error_move),
            toastType: ToastType.error,
          );
        }
      }
    ...
    Future<void> deleteFolder(SpaceAlbumFolder folder) async {
      ...
      try {
        ...
        if (context.mounted) {
          ImmichToast.show(
            context: context,
            msg: _folderErrorMessage(context, error, context.t.space_album_folder_error_delete),
            toastType: ToastType.error,
          );
        }
      }
        ...
        actions: [
          if (canEdit) ...[
            IconButton(
              ...
              tooltip: context.t.space_album_folder_new,
            ),
            IconButton(
              key: const Key('space-albums-new-album-action'),
              onPressed: createAlbum,
              icon: const Icon(Icons.photo_album_outlined),
              tooltip: context.t.space_album_new,
            ),
            IconButton(
          ...
          Future<void> moveAlbumToFolder(SpaceAlbum album) async {
            ...
            try {
              ...
              if (context.mounted) {
                ImmichToast.show(
                  context: context,
                  msg: context.t.space_album_folder_error_move,
                  toastType: ToastType.error,
                );
              }
            }
...
/// Resolves an API error to its translated message.
///
/// ROLLING: `context.t` is a typed accessor with no `t[key]` indexer, so the key that
/// `_folderErrorKey` computes at runtime cannot be looked up dynamically. The three keys it can
/// return are enumerated here instead; anything else falls back to the caller's already-translated
/// string. Keep this switch in step with `spaceAlbumFolderErrorKey`.
String _folderErrorMessage(BuildContext context, Object error, String fallback) => switch (_folderErrorKey(error, '')) {
  'space_album_folder_depth_exceeded' => context.t.space_album_folder_depth_exceeded,
  'space_album_folder_limit_reached' => context.t.space_album_folder_limit_reached,
  'space_album_folder_name_taken' => context.t.space_album_folder_name_taken,
  _ => fallback,
};
    ...
    return AlertDialog(
      ...
      content: SingleChildScrollView(
        child: TextFormField(
          ...
          autofocus: true,
          decoration: InputDecoration(labelText: context.t.space_album_folder_name_label),
...
Future<bool> _confirmDeleteFolder(BuildContext context, SpaceAlbumFolder folder) async {
  final confirmed = await showDialog<bool>(
    ...
    builder: (ctx) => AlertDialog(
      title: Text(ctx.t.space_album_folder_delete),
      content: Text(ctx.t.space_album_folder_delete_confirm(name: folder.name)),
      actions: [
        TextButton(
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 16),
          Text(
            context.t.space_album_folder_empty,
            style: context.textTheme.titleMedium?.copyWith(color: context.colorScheme.onSurfaceVariant),
          ),
        ],
      ),
