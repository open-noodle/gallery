...
class AuthenticationApi {
  ...
  ///
  /// Note: This method returns the HTTP [Response].
  Future<Response> demoLoginWithHttpInfo({ Future<void>? abortTrigger, }) async {
    // ignore: prefer_const_declarations
  ...
  ///
  /// Login as the demo user. Only available when demo mode is enabled.
  Future<LoginResponseDto?> demoLogin({ Future<void>? abortTrigger, }) async {
    final response = await demoLoginWithHttpInfo(abortTrigger: abortTrigger,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
