...
> **⚠️ SPIKE COMPLETE (2026-07-24) — read the results first: [`2026-07-24-pet-recognition-phase1-RESULTS.md`](2026-07-24-pet-recognition-phase1-RESULTS.md).** The spike ran and superseded this plan's model recipe. Verdict: **GO**. The winning recipe is **NOT** the three configs below (zero-shot / head / full) — full fine-tuning overfits, and the "head" config is a no-op (it trains a classifier that's discarded at inference, leaving the embedding unchanged). The winner is a **frozen DINOv2 backbone + a trained _projection head_** (the projection becomes the embedding) at three backbone sizes (small/base/large). Phase 2 should redesign the "head" config as this projection head. The data-layer tasks (parsers, manifest, split), eval metrics, and ONNX export below remain valid; the model/train tasks are superseded by the projection recipe in the results doc.
