...
Both toggles are already settable from `IMMICH_CONFIG_FILE` with no additional code: `buildConfig` (`server/src/utils/config.ts:76-119`) loads the file and deep-merges it over `defaults`. Validity is decided by `SystemConfigSchema.safeParse` (`config.ts:106`) — a non-strict Zod object that silently strips any key the schema doesn't declare, while an unknown key merely logs a warning (`config.ts:97-103`) and is never stripped or thrown on. So it's `SystemConfigSchema` declaring `storageUsage` (Task 1) that makes the key pass through the merge into the validated config; a config-file test pins that side. The `defaults` side — that a missing key falls back to `false`/`false` rather than `undefined` — is a separate guarantee, already pinned by Task 1's `should default both derivative toggles to off (upstream behavior)` test in the same spec file, which calls `getConfig({ withCache: false })` with no config file and would fail if `storageUsage` were dropped from `defaults`. This task documents the config-file surface and adds the config-file-path test, so a future refactor cannot silently make the key unknown again on that path.
