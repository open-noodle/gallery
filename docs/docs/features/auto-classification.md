...
The SigLIP2 models above all sit in the same ~3-4 GiB memory band, so the real tradeoff between them is inference time. If RAM is your constraint and CPU is not, drop to a `ViT-B-16-SigLIP*` model instead of a smaller SO400M variant.
...
3. Wait for Smart Search to finish. Classification skips any asset that has no embedding
...
Negative-sounding prompts ("no people, no faces, no landscapes") are a common trick, but be clear on what they actually do. CLIP has no notion of negation, so such a prompt is simply another vector an asset can match against. In practice they work as _contrastive_ prompts: they tend to pull the category's best-match score toward text-heavy, non-photographic images. They exclude nothing. For genuine exclusion, use [Face exclusion](#face-exclusion) instead.

### A real-world example

The configuration below comes from a Gallery user running auto-classification over a large personal library ([discussion #795](https://github.com/open-noodle/gallery/discussions/795)). It is a useful starting point because it shows realistic prompt density and per-category similarity values.

Treat the thresholds as a starting point. They were tuned for that user's model and library. See [Choosing a CLIP model](#choosing-a-clip-model) and the [similarity threshold](#similarity-threshold) notes above.
...
- Tag only (`tag`): matching photos get an `Auto/{category name}` tag and stay on your timeline.
- Tag and archive (`tag_and_archive`): matching photos get tagged and moved to the Archive. Useful for screenshots or receipts you want organized and off your timeline.
...
This queues all assets across all users for classification. It is additive: existing tags are kept, and new matches get tagged.

## Troubleshooting

### Nothing is being tagged

This is the most common complaint, and it is almost always a tuning problem. Work through these in order:
...
Classification runs right after Smart Search processes a photo. The CLIP model compares the photo's visual embedding against your category prompts. If the best-matching prompt clears the similarity threshold, the photo goes into that category.
...
UI / API updates (`onConfigUpdate`) fire immediately when an admin saves via the UI or API:
...
Config file (`onConfigInit`) fires on every server boot, including after a YAML edit and restart:
...
The snapshot is the bridge between the two paths. Both paths write it after running cleanup, so the boot-time check never re-runs cleanup that was already handled by the UI path. On the very first boot after upgrading, the snapshot is missing. Gallery stores a baseline and skips cleanup, so existing tags are preserved.

### Key Details

- Cosine similarity is computed in-process (dot product / magnitude product), never through a database query
- `scanLibrary` streams unclassified assets and queues individual jobs in batches of 1,000
- Tagging is idempotent: re-classification upserts, so it never duplicates a tag
- The global kill switch `classification.enabled: false` short-circuits both the queue-all job and individual classify jobs, processing no assets at all
- Category names must be unique, enforced by DTO validation
- If the ML service is down, individual classify jobs fail and BullMQ retries them. Failed encodes are not cached, so the next attempt re-tries the ML call.
