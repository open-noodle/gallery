...
The cost of that caution is a long tail of near-misses in every library. The profile shot, the one in bad light, the one from four years ago: all unassigned, while the person they belong to is named and obvious.
...
The difference between the last two is worth knowing. **Different person** is a real verdict: it is stamped against the person's identity, so it answers "not this person" everywhere that identity is checked, including rows you were not looking at. **Ignore face** only stops the asking.
...
Owners and Editors get suggestions, because they are the members who can assign faces. **Viewers get none**, since a queue of questions they cannot answer would only be noise.
...
:::note
**Suggestion max distance** must be greater than **Maximum recognition distance**. A smaller value can never match anything, because every candidate would already have been auto-assigned, so the settings page refuses to save it. Suggestions also require facial recognition itself to stay enabled.
:::
...
Anything left pending when you switched off is re-validated before it is shown again, so a suggestion that went stale in the meantime (the face was assigned to someone, the photo was deleted, the suggestion distance was narrowed) drops out quietly instead of coming back.
