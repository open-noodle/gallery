...
Pet Recognition tells your individual pets apart. Turn it on and two dogs stop being one shared "dog" entry: they become **Rocky** and **Luna**, two people you can name, rename, merge and browse.
...
That is a limit of the model. It is trained on dog and cat identities, so it has no basis for telling one bird apart from another, which is exactly why recording those species had nowhere useful to go.
:::

## Enabling it

Pet Recognition is **off by default**. Turning it on is not enough on its own: see [Reprocessing your library](#reprocessing-your-library) below.
...
From that point on, **newly uploaded** photos go through the individual pipeline. Photos already in your library keep whatever they had, so until you reprocess the library sits in a mixed state: old photos still attached to the shared "dog" entry, which is not listed in the People section, while new ones appear there as named-able individuals.
...
Gallery then deletes all pet people and embeddings and re-runs pet detection over the whole library, embedding and clustering as it goes. On a large library this takes a while, since it re-runs the detector on every asset.
...
Three models are available. All produce the same 512-dimension embedding, so you can switch between them without a schema change. Larger models are more accurate and slower, and the download is not small.
...
- **Lower it** if Gallery is merging two different pets into one, which happens with two dogs of the same breed and colour.
...
Raising it makes recognition more precise, but a pet with only a photo or two then never becomes an individual at all. The default of 1 suits most home libraries, where a pet may appear in only a handful of photos.
