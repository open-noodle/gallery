...
If you are using Cloudflare Tunnel, please know that they set a maximum request body size of 100 MB that cannot be changed.
This is no longer a ceiling on the size of the asset you can upload — Gallery uses [chunked/resumable upload](/features/chunked-upload) for files larger than 32 MiB, so a large photo or video is sent as a sequence of 32 MiB chunks, each well under Cloudflare's 100 MB limit, rather than as one request for the whole file. The 100 MB limit only matters if it is smaller than a single chunk, which it is not.
Note that upload resumption is within the same browser tab or app session only: if the upload is interrupted by closing the tab or app, that file restarts from the beginning on the next attempt rather than picking up where it left off.
If you are still having issues, we recommend switching to a different network deployment.
...
The edit appears to work at first: Gallery applies it, then reverts it when the file is re-read, and shows no error.
...
We recommend the PostgreSQL version from our default docker-compose.yml. Newer versions work too, but a major version upgrade takes more than swapping the container image tag. See [the PostgreSQL documentation](https://www.postgresql.org/docs/current/upgrading.html) for the steps.
