...
The `storage.routing` section configures [per-file-type storage routing](/features/s3-storage#choosing-where-each-file-kind-is-stored) — which backend, disk or S3, each kind of newly written file goes to. The same values are available in **Administration > System Settings > Storage routing** when no config file is in use, but that page is unreachable on a config-file install — `updateAdminConfig` rejects outright when `IMMICH_CONFIG_FILE` is set — so the config file is the only way to set these on such an install.
