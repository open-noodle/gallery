...
describe('detectAlbums', () => {
  ...
  it('returns empty array when no albums detected', () => {
    const items: TakeoutMediaItem[] = [
      { path: 'photo1.jpg', file: new File([], 'photo1.jpg') },
    ];
...
describe('scanTakeoutFiles', () => {
  ...
  it('supports abort signal for cancellation', async () => {
    ...
    await expect(
      scanTakeoutFiles({ files: [zip], signal: controller.signal }),
    ).rejects.toThrow();
  });
});
```
...
const MEDIA_EXTENSIONS = new Set([
  ...
  '.bmp', '.avif', '.jxl', '.raw', '.arw', '.cr2', '.cr3', '.dng', '.nef',
  '.orf', '.raf', '.rw2', '.srw', '.pef',
...
describe('ImportManager', () => {
  ...
  describe('album selection', () => {
    it('toggles album selection', () => {
      manager.setAlbums([
        { name: 'Trip', itemCount: 5, isAutoGenerated: false, itemPaths: [] },
      ]);
...
export class ImportManager {
  ...
  setAlbums(albums: TakeoutAlbum[]) {
    this.selectedAlbums = new Set(
      albums.filter((a) => !a.isAutoGenerated).map((a) => a.name),
    );
  }
