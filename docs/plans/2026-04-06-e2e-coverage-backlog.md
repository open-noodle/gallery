...
- **Space person `pet` filter applies at BOTH the listing AND the single-person endpoint** — turning `shared_space.petsEnabled=false` not only hides pets from `GET /people` but also rejects `GET /people/:personId` with 400 for any pet personId. The `hidden` filter has no equivalent — hidden persons remain addressable via direct fetch. Asymmetry pinned by T10. UX consequence: turning pets off makes the entire pet sub-graph invisible to all members.
- **Single-person endpoints return 400 (not 404) for unknown personIds** — `getSpacePerson` routes through `requireAccess({permission: SharedSpaceRead, ids: [personId]})` which uses Immich's bulk-access pattern. The same 400-not-404 behaviour applies to PUT/DELETE/merge sub-endpoints. Pinned by T10. T11+ tests should follow the same convention.
...
- **Space person thumbnail returns 500 when the fixture thumbnailPath is fictional** — `utils.createSpacePerson` sets `person.thumbnailPath = '/my/awesome/thumbnail.jpg'` to satisfy the listing's hard requirement, but the file doesn't exist on disk in the test stack. The thumbnail endpoint passes the access check then fails at file resolution and returns 500 instead of a graceful 404. T10's thumbnail access matrix pins `spaceOwner: 500` accordingly. The 500 itself is a small server-side bug worth fixing but out of scope for the coverage backlog. Pinned by T10.
